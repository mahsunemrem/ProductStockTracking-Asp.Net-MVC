...
namespace ProductStockTracking.DataAccess.Concrete.EntityFramework.Contexts
{
    public class ProductStockTrackingContext : DbContext
    {
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<UserRole>().HasRequired(c => c.User).WithMany(i => i.UserRoles).HasForeignKey(k => k.UserId);
            modelBuilder.Entity<UserRole>().HasRequired(c => c.Role).WithMany(i => i.UserRoles).HasForeignKey(k => k.RoleId);
